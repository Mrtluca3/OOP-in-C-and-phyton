...
int main(int argc, char* argv[]) {
  ...
  // 1) name of the ROOT file where the tree is stored ("~/OOP_physics/execute/data.root")
  // 2) name of the tree object ("datatree")
  // 3) name of the branches ("value" and "error")
  TString rootfname("~/OOP_physics/execute/mydata4.root"); // [1]
  ...
  TTree* tree = (TTree*) infile->Get("mydatatree3"); // [2]
  if(!tree) {
  ...
  for (int i=0; i<nentries; ++i) {
    ...
    tree->GetEntry(i);
    // Fill in histograms with value and error data
    hx1.Fill(d->value());
    hdx1.Fill(d->error());
